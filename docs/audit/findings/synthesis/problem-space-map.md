...
| AL-16 | **UDP multicast discovery + heartbeat eviction** — announce `{DeviceID, addr, port}`; evict silent peers | LAN peer discovery (**a hint, never authorisation**) | `internal/discovery` | **ADOPT** | `structure.md` discovery; SKILL §7; `syncthing-source` D3-4 |
